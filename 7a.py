...
for line in theInput:
    hypernetABBAFound = False
    ABBAfound = False
    lineLength = len(line)
    for i in range(1, lineLength-2):
        if line[i] == line[i+1] and line[i-1] == line[i+2] and line[i-1] != line[i]:
            ABBAfound = True
            # Try to find if there's brackets
            rIndex = 3
            while(i+rIndex < lineLength):
                if line[i+rIndex] == '[':
                    break;
                elif line[i+rIndex] == ']':
                    hypernetABBAFound = True
                    break;
                rIndex+=1
    if ABBAfound and not hypernetABBAFound:
        print(line)
        totalTLS += 1
print(totalTLS)
