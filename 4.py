...
# room = encrypted name: lowercase+ dashed, then a dash, then a sector ID, then a checksum in []'s
#room = real if checksum is 5 most common letters in the encrypter name in orders, with ties broken by alphabetization
# goal: sum of the secotor ID from the real rooms

theInput = """fubrjhqlf-edvnhw-dftxlvlwlrq-803[wjvzd]
...
#theInput = """aaaaa-bbb-z-y-x-123[abxyz]
#a-b-c-d-e-f-g-h-987[abcde]
#not-a-real-room-404[oarel]
#totally-real-room-200[decoy]"""

theInputLines = theInput.split('\n')
totalSum = 0
for line in theInputLines:
    lineList = list(line.split('-'))
    checksum = list(lineList[-1])
    checksum.reverse()
    checksumLetters = checksum[1:6]
    id = checksum[7:10]
    id.reverse()
    checksumLetters.reverse()
    theString = ''.join(lineList[0:len(lineList)-1])
    #print(checksumLetters)
    #print(id)
    #print(theString)
    dict4Checksum = dict()

    for letter in theString:
        dict4Checksum[letter] = theString.count(letter)
