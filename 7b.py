...
theInput= """xdsqxnovprgovwzkus[fmadbfsbqwzzrzrgdg]aeqornszgvbizdm
...
for line in theInput:
    splitLineList = re.split('\[|\]', line)
    supernetList = splitLineList[0::2]
    hypernetList = splitLineList[1::2]
    ABAList = []
    BABList = []
    for supernet in supernetList:
        for i in range(len(supernet)-2):
            if supernet[i] == supernet[i+2] and supernet[i+1] != supernet[i]:
                ABAList.append(supernet[i:i+3])
    for hypernet in hypernetList:
        for i in range(len(hypernet)-2):
            if hypernet[i] == hypernet[i+2] and hypernet[i+1] != hypernet[i]:
                BABList.append(hypernet[i:i+3])
    #print(supernetList, end=', ')
    #print(hypernetList, end=', ')
    #print(ABAList, end=', ')
    #print(BABList)
    for ABA in ABAList:
        wantedBAB = ABA[1]+ABA[2]+ABA[1] 
        if wantedBAB in BABList:
            totalTLS +=1
            #print('boop')
            break
