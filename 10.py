...
theInput = """bot 49 gives low to bot 118 and high to bot 182
...
valueMatcher = re.compile('value (\d+) goes to bot (\d+)')
commandMatcher = re.compile('bot (\d+) gives low to (bot|output) (\d+) and high to (bot|output) (\d+)')
...
for line in theInput:
    if line[0:5] == 'value':
        aMatch = valueMatcher.match(line)
        if botValueDict.get(int(aMatch.group(2))) == None:
            botValueDict[int(aMatch.group(2))] = [int(aMatch.group(1))]
        else:
            botValueDict[int(aMatch.group(2))].append(int(aMatch.group(1)))
    else:
        aMatch = commandMatcher.match(line)
        if botInstrDict.get(int(aMatch.group(1))) == None:
            botInstrDict[int(aMatch.group(1))] = [int(aMatch.group(3)), int(aMatch.group(5))]
            botInstrType[int(aMatch.group(1))] = [aMatch.group(2), aMatch.group(4)]
        else:
            print('wtf!')
            input()

while max(len(botValueDict[i]) for i in botValueDict.keys()) == 2:
    theKeys = list(botValueDict.keys())
    for botKey in theKeys:
        if len(botValueDict[botKey]) == 3:
            print('error!')
            input()
        if len(botValueDict[botKey]) == 2:
            minValue = min(botValueDict[botKey])
            maxValue = max(botValueDict[botKey])
            minTag = botInstrDict[botKey][0]
            maxTag = botInstrDict[botKey][1]
            minDir = botInstrType[botKey][0]
            maxDir = botInstrType[botKey][1]
            if minDir == 'bot':
                if botValueDict.get(minTag) == None:
                    botValueDict[minTag] = []
                botValueDict[minTag].append(minValue)
            elif minDir == 'output':
                if binsDict.get(minTag) == None:
                    binsDict[minTag] = []
                binsDict[minTag].append(minValue)
            if maxDir == 'bot':
                if botValueDict.get(maxTag) == None:
                    botValueDict[maxTag] = []
                botValueDict[maxTag].append(maxValue)
            elif maxDir == 'output':
                if binsDict.get(maxTag) == None:
                    binsDict[maxTag] = []
                binsDict[maxTag].append(maxValue)

            botValueDict[botKey] = []
            if minValue == 17 and maxValue == 61:
                print('The bot who compares 17 and 61 is ' + str(botKey))
        theKeys = botValueDict.keys()

theMultValue = 1
for aKey in binsDict.keys():
    if aKey in (0, 1, 2):
        theMultValue *= binsDict[aKey][0]
print('the product of the values in bins 0 1 and 2 is equal to ' + str(theMultValue))
